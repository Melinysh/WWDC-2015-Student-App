...
-(void)beginGame{
    ...
    isGameReady = YES;
    NSLog(@"Game load completed.\n %lu balls added \n %lu bombs added \n isGameready: %hhd", (unsigned long)balls.count, (unsigned long)bombs.count, isGameReady);
...
-(void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    for (UIImageView *view in balls)
    {
        if([view.layer.presentationLayer hitTest:touchPoint] && isGameReady == YES) //tests if the animated image of the ball if hit
        {
            ...
            NSLog(@"balls.count: %lu (%hhd)", (unsigned long)balls.count, isGameReady);
    ...
    if(balls.count == 0 && isGameReady == YES){ //if no ball images left
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Winner!" message:btnTitle delegate:self cancelButtonTitle:@"Play Again" otherButtonTitles:@"Main Menu", nil]; //inits and shows alert
        [alert addButtonWithTitle:@"Share"];
        alert.tag = 1; //to recognize which alert when button pressed
        [alert show];
        NSLog(@"Alert shown.");
    ...
    for (UIImageView *view in bombs)
    {
        if([view.layer.presentationLayer hitTest:touchPoint ] && hasBallBeenRemoved == NO) //tests if the animated image of the ball if hit
        {
            ...
            [UIView animateWithDuration:3.0
                             ...
                             completion:nil];
            UIAlertView *gameOverAlert = [[UIAlertView alloc]initWithTitle:@"Game Over!" message:@"Oops! You hit a bomb!" delegate:self cancelButtonTitle:@"Main Menu" otherButtonTitles:@"Play again", nil];
            gameOverAlert.tag =2;
            [gameOverAlert show];
...
//Pop-up alert view when game is finished. This is execute if the button
//pressed is play again or not, depending on btn index. Returns to main menu if 'no' pressed.
-(void) alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    
    if (alertView.tag == 1) //WINNER ALERT VIEW
    {
        if(buttonIndex == 1)
        {
    ...
    } else if (alertView.tag == 2) //GAME OVER ALERT VIEW
    {
        if (buttonIndex == 1) {
            NSLog(@"Play again button pressed.");
           // [self cleanUpBombs:YES Balls:YES];
            ...
            NSLog(@"Main menu button pressed.");
           // [self cleanUpBombs:YES Balls:YES];
			isGameReady = NO;
            [self dismissViewControllerAnimated:true completion:nil];
        }
